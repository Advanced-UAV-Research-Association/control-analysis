...
# %% [markdown]
# # Control System Design (Poster Version)
# Here we will design the control system for the plane, we will use LQR to find the optimal poles
# This version produces combined plots suitable for poster display
...
# %% [markdown]
# ### Open Loop Pitch Axis Response (Combined Plot)
...
# Angle variables on primary axis (right side) - alpha (i=0) and theta (i=2)
alpha_deg = np.rad2deg(response_pitch.y[0])
theta_deg = np.rad2deg(response_pitch.y[2])
ax.plot(response_pitch.time, alpha_deg, color='ch1', linewidth=2, label='Angle of Attack (deg)')
ax.plot(response_pitch.time, theta_deg, color='ch3', linewidth=2, label='Pitch Angle (deg)')

# Rate variable on secondary axis (left side) - q (i=1)
q_deg = np.rad2deg(response_pitch.y[1])
ax2.plot(response_pitch.time, q_deg, color='ch2', linewidth=2, label='Pitch Rate (deg/s)')

ax.set_xlabel('Time (s)', fontsize=14)
ax.set_ylabel('Angle (deg)', fontsize=14)
ax2.set_ylabel('Pitch Rate (deg/s)', fontsize=14)
ax.grid(True, alpha=0.3)
ax.set_title('Open Loop Pitch Axis Response', fontsize=16, fontweight='bold')

# Align zero points on both y-axes
y1_low, y1_high = ax.get_ylim()
y2_low, y2_high = ax2.get_ylim()
# Ratio of positive to negative range on primary axis
ratio = y1_high / abs(y1_low) if y1_low < 0 else 1
# Compute secondary limits that align zero AND include all data
# y2_new_high / abs(y2_new_low) = ratio, and limits must cover [y2_low, y2_high]
...
# %% [markdown]
# ### Open Loop Roll Axis Response (Combined Plot)
...
phi_deg = np.rad2deg(response_roll.y[1])
ax.plot(response_roll.time, phi_deg, color='ch2', linewidth=2, label='Roll Angle (deg)')

# Rate variable on secondary axis (left side) - p (i=0)
p_deg = np.rad2deg(response_roll.y[0])
ax2.plot(response_roll.time, p_deg, color='ch1', linewidth=2, label='Roll Rate (deg/s)')

ax.set_xlabel('Time (s)', fontsize=14)
ax.set_ylabel('Angle (deg)', fontsize=14)
ax2.set_ylabel('Roll Rate (deg/s)', fontsize=14)
ax.grid(True, alpha=0.3)
ax.set_title('Open Loop Roll Axis Response', fontsize=16, fontweight='bold')
...
# %% [markdown]
# ### Closed Loop Pitch Axis Response (Combined Plot)
...
# Angle variables on primary axis (right side) - alpha (i=0) and theta (i=2)
alpha_deg_cl = np.rad2deg(response_cl_pitch.y[0])
theta_deg_cl = np.rad2deg(response_cl_pitch.y[2])
ax.plot(response_cl_pitch.time, alpha_deg_cl, color='ch1', linewidth=2, label='Angle of Attack (deg)')
ax.plot(response_cl_pitch.time, theta_deg_cl, color='ch3', linewidth=2, label='Pitch Angle (deg)')

# Rate variable on secondary axis (left side) - q (i=1)
q_deg_cl = np.rad2deg(response_cl_pitch.y[1])
ax2.plot(response_cl_pitch.time, q_deg_cl, color='ch2', linewidth=2, label='Pitch Rate (deg/s)')
...
# %% [markdown]
# ### Closed Loop Roll Axis Response (Combined Plot)
...
# %% [markdown]
# ### Control Surface Deflections (Combined Plot)

# %%
fig, ax = plt.subplots(figsize=(12, 8))

# Pitch deflection
ax.plot(t_out_pitch, np.rad2deg(u_out_pitch[0]), color='ch3', linewidth=2,
        label='Elevator Deflection (deg)')

# Roll deflection
ax.plot(t_out_roll, np.rad2deg(u_out_roll[0]), color='ch1', linewidth=2,
         label='Aileron Deflection (deg)')

ax.set_xlabel('Time (s)', fontsize=14)
ax.set_ylabel('Control Deflection (deg)', fontsize=14)
ax.grid(True, alpha=0.3)
ax.set_title('Control Surface Deflections', fontsize=16, fontweight='bold')
